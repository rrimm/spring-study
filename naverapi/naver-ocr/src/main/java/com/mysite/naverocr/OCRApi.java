...
public class OCRApi {
    public void processOCR(String img) {
        ...
        try {
            ...
            con.setDoInput(true);   // InputStream으로 서버로부터 응답받겠다는 옵션
            ...
            while ((inputLine = br.readLine()) != null) { // 응답 수신
    ...
    private void responseDataPrint(String response) {
        ...
        JSONObject title = (JSONObject) imagesObj.get("title"); // 대표 샘플명
        JSONArray fields = (JSONArray) imagesObj.get("fields"); // 필드
        for (int i = 0; i < fields.length(); i++) {
            JSONObject fieldsObj = (JSONObject) fields.get(i);
            String name = fieldsObj.getString("name");  // 필드명
            String inferText = fieldsObj.getString("inferText");    // OCR로 추출하는 실제 값
            System.out.println("--------------------------------------");
